...
REM    5. drops photon_pun_mono.dll into <game>\plugins\
...
echo  your Photon Voice AppId GUID below. If no (game doesn't have
echo  voice chat), just press Enter to skip.
...
if not %RC%==0 (
  ...
  echo ============================================================
  echo.
  echo  The script could not find a Photon AppId GUID in
  echo  this game's assets. Possible causes:
  echo    - Wrong folder picked (must contain the .exe and the
  echo      ^<Game^>_Data subfolder)
  echo    - Game does not use Photon PUN
  echo    - Game uses IL2CPP, not Mono -- try the photon_pun
  echo      plugin's Setup.bat instead
  echo.
  pause
  exit /b 1
)
...
echo.
echo ============================================================
echo   2/3  Copying photon_pun_mono.dll into the game folder...
echo ============================================================
echo.
