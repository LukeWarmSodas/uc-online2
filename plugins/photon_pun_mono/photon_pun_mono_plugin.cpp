...
//
// Mono-runtime counterpart of plugins/photon_pun. Unity games
                            ...
                            const char* logName)
{
    ...
    if (!fn) { LOG("[PUN/Mono] could not find %s", logName); return false; }
    if (MH_CreateHook(fn, hook, orig) != MH_OK || MH_EnableHook(fn) != MH_OK)
    {
        LOG("[PUN/Mono] hook FAILED for %s at %p", logName, fn);
        return false;
    }
    LOG("[PUN/Mono] %s hook installed at %p", logName, fn);
    return true;
}
...
static void EnsureOurAppIdString()
{
    ...
    if (g_OurAppIdString)
        LOG("[PUN/Mono] built managed string for '%s' at %p",
...
static void* PatchAppIdArg(void* appId, const char* sender)
{
    ...
    if (appId != g_OurAppIdString)
        LOG("[PUN/Mono] %s: appId arg %p -> %p", sender, appId, g_OurAppIdString);
...
static void PatchAuthValuesAuthType(void* authValues, const char* sender)
{
    ...
    *p = (unsigned char)(g_ForcedAuthType & 0xFF);
    LOG("[PUN/Mono] %s: authValues.authType %u -> %u", sender, prev, g_ForcedAuthType);
}
...
{
    LOG("[PUN/Mono] DIAG: LoadBalancingClient.ConnectToMasterServer called");
...
{
    LOG("[PUN/Mono] DIAG: LoadBalancingClient.ConnectToNameServer called");
...
{
    LOG("[PUN/Mono] DIAG: LoadBalancingClient.ConnectUsingSettings called");
...
{
    LOG("[PUN/Mono] DIAG: PhotonNetwork.ConnectUsingSettings called");
...
{
    LOG("[PUN/Mono] DIAG: LoadBalancingClient.AuthenticateAsync called");
                                             ...
                                             void* sendOptions, void* a5, void* a6)
{
    ...
    LOG("[PUN/Mono] SendOperation opCode=%u (%s) params=%p", opCode, opName, params);
    ...
    if (isAuth && params)
    {
        ...
        if (g_ForcedAuthType <= 255)
        {
            ...
            {
                LOG("[PUN/Mono] SendOperation: rewrote params[217] AuthType -> %u",
                    g_ForcedAuthType);
            }
        }

        // ALSO rewrite ApplicationId = our user's Realtime GUID, in case
        // REPO has the AppId hard-coded in code that ignores the asset.
        if (g_AppIdPatchEnabled && g_OurAppIdUtf8[0])
        {
            if (MONO_DictByteStringSetItem((MonoObject*)params, 224, g_OurAppIdUtf8))
            {
                LOG("[PUN/Mono] SendOperation: rewrote params[224] AppId -> %s",
                    g_OurAppIdUtf8);
            }
            else
            {
                LOG("[PUN/Mono] SendOperation: FAILED to rewrite params[224]");
            }
        }
    }
...
static void LogBytesAt(void* addr, const char* label)
{
    ...
    LOG("[PUN/Mono] BYTES @ %s (%p): %s", label, addr, buf);
}
...
static DWORD WINAPI WatcherProc(LPVOID)
{
    ...
    if (!InterlockedCompareExchange(&g_bShutdown, 0, 0))
        LOG("[PUN/Mono] mono-2.0-bdwgc.dll never resolved -- giving up");
    return 0;
}
...
extern "C" __declspec(dllexport) int __cdecl UCO_PluginInit(const UCO_PluginContext* ctx)
{
    ...
    LOG("[PUN/Mono] plugin v1 init: AppId=%u ogAppId=%u",
    ...
    if (ini)
    {
        ...
        if (g_OurAppIdUtf8[0])
        {
            ...
            LOG("[PUN/Mono] PhotonAppIdRealtime override set: %s", g_OurAppIdUtf8);
        }
        else
        {
            LOG("[PUN/Mono] no [PUN]PhotonAppIdRealtime in ini -- AppId patch disabled");
        }
        ...
        LOG("[PUN/Mono] forced AuthType = %u", g_ForcedAuthType);
    }

    if (MH_Initialize() != MH_OK)
    {
        LOG("[PUN/Mono] MH_Initialize non-OK (already inited?)");
    }
