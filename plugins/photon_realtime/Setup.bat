...
REM    4. drops photon_pun.dll into <game>\plugins\
REM    5. writes a complete union-crax.ini at <game>\
REM
REM  The game's resources.assets is NOT modified. The plugin
REM  rewrites the AppId GUID on the wire at runtime (via the
REM  LoadBalancingPeer.OpAuthenticate and PhotonPeer.SendOperation
REM  hooks), so the ini is the single source of truth for which
REM  Photon app the game authenticates against. If you change
REM  the GUID later, just edit union-crax.ini -- no re-patching.
REM
...
echo.
echo ============================================================
echo   1/2  Copying photon_pun.dll into the game folder...
echo ============================================================
echo.
